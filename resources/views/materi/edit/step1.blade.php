...
@section('content')
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('materi.update.step1', $materi->id) }}" id="createMaterialForm" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')

            @if ($materi->cover_image)
                <input type="hidden" name="existing_cover_image" value="{{ $materi->cover_image }}">
            @endif
                    ...
                    <div
                        ...
                        <div class="p-6 md:p-8 space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title', $materi->title) }}"
                                ...
                                <div class="flex justify-end mt-1.5">
                                    <span id="title-counter"
                                        class="text-xs text-slate-400 font-medium">{{ strlen($materi->title) }}/100
                                        karakter</span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea rows="4" name="description"
                                    ...
                                    placeholder="Jelaskan ringkasan materi apa yang akan dipelajari...">{{ old('description', $materi->description) }}</textarea>
                                @error('description')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="p-6 md:p-8 space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <select name="category_id"
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}"
                                                    {{ old('category_id', $materi->category_id) == $category->id ? 'selected' : '' }}>
                                                    {{ $category->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="topic" value="{{ old('topic', $materi->topic) }}"
                            ...
                            <div id="upload-container" class="relative">
                                <label id="upload-label"
                                    ...
                                    <div id="preview-container"
                                        class="preview-container {{ $materi->cover_image ? '' : 'hidden' }}">
                                        @if ($materi->cover_image)
                                            <img id="image-preview" class="preview-image"
                                                src="{{ asset('storage/' . $materi->cover_image) }}"
                                        ...
                                        <div id="file-info" class="file-info">
                                            @if ($materi->cover_image)
                                                ...
                                                <span>{{ basename($materi->cover_image) }}</span>
                                            @endif
                                        </div>
                                    </div>
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200 p-6 space-y-6">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="is_premium" value="1" class="sr-only peer"
                                    {{ old('is_premium', $materi->is_premium) ? 'checked' : '' }}>
                                <div
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" name="allow_comments" value="1" class="sr-only peer"
                                    {{ old('allow_comments', $materi->allow_comments) ? 'checked' : '' }}>
                                <div
                                    class="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                                </div>
                            </label>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // Inisialisasi untuk gambar yang sudah ada
                if ('{{ $materi->cover_image }}') {
                ...
                fileInput.addEventListener('change', function(e) {
                    ...
                    if (file) {
                        ...
                        `;
                            ...
                            // Hapus input hidden remove_cover_image jika ada
                            const removeCoverImage = document.getElementById('remove_cover_image');
                            if (removeCoverImage) {
                ...
                removePreview.addEventListener('click', function(e) {
                    ...
                    if (!document.getElementById('remove_cover_image')) {
                        const removeInput = document.createElement('input');
                        removeInput.type = 'hidden';
                        removeInput.id = 'remove_cover_image';
                        removeInput.name = 'remove_cover_image';
                        removeInput.value = '1';
                        document.getElementById('createMaterialForm').appendChild(removeInput);
                    }

                    resetPreview();
                    fileInfo.innerHTML = '<span>Gambar akan dihapus</span>';
                });
                ...
                const charCounter = document.getElementById('title-counter');
